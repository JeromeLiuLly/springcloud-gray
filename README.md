...
![metadata管理页面](https://raw.githubusercontent.com/JeromeLiuLly/springcloud-gray/simple/SpringCloudGray/metadata.png)

## 项目结构
### config 配置中心
端口：6007，方便起见直接读取配置文件，生产环境可以读取git。先启动配置中心，所有服务的配置（包括注册中心的地址）均从配置中心读取。

### gray-order-service 服务消费者
端口：6011，调用服务提供者和服务提供者，为了演示header传递。

### gray-order-service2 服务消费者【可这种成灰度服务】
端口：6012，调用服务提供者，为了演示header传递。

### core 框架核心包
核心jar包，所有微服务均引用该包，使用AutoConfig实现免配置，模拟生产环境下spring-cloud的使用。

### eureka 注册中心
端口：6001，/metadata端点实现metadata信息配置。

### gray-user-service 服务消费者
端口：6010，调用服务提供者和服务提供者，为了演示header传递。

### gray-user-service2 服务消费者【可这种成灰度服务】
端口：6013，调用服务提供者，为了演示header传递。

### zuul 网关
端口：4002，演示解析token获得label并放入header往后传递
